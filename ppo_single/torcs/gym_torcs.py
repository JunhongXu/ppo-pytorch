...
class TorcsEnv:
    ...
    def __init__(self, vision=False, throttle=False, brake=False, gear_change=False):
        self.vision = vision
        ...
        elif throttle is True and brake is True:
            self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(3,))
        elif throttle is True and brake is False:
    ...
    def step(self, u):
       #print("Step")
        ...
        obs = client.S.d
        # print('raced', obs['distRaced'] - obs_pre['distRaced'])
        # print('from start', obs['distFromStart'] - obs_pre['distFromStart'])
        ...
        if obs['distRaced'] - obs_pre['distRaced'] < 0.5: # if the agent is barely moving
            dist_rwd = -1
        else:
            dist_rwd = min(2, obs['distRaced'] - obs_pre['distRaced'])

        progress = sp * np.cos(obs['angle']) \
                   # - np.abs(sp * np.sin(obs['angle'])) - \
    ...
    def make_observaton(self, raw_obs):
        if self.vision is False:
            ...
            return Observation(focus=np.array(raw_obs['focus'], dtype=np.float32)/200.,
                               ...
                               rpm=np.array(raw_obs['rpm'], dtype=np.float32)/10000,    # modified
                               track=np.array(raw_obs['track'], dtype=np.float32)/200.,
                               wheelSpinVel=np.array(raw_obs['wheelSpinVel'], dtype=np.float32),
                               angle=np.array(raw_obs['angle'], dtype=np.float32)/np.pi,    # modified, -pi, pi
