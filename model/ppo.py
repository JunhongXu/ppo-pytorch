...
def ppo_update(policy, optimizer, batch_size, memory, nupdates, epoch,
               clip_value=0.2, coeff_entropy=0.01, writer=None):
    ...
    for update in range(nupdates):
        ...
        for i, index in enumerate(sampler):
            ...
            sampled_actions = Variable(torch.from_numpy(actions[index]))\
                .float().cuda()
            sampled_logprobs = Variable(torch.from_numpy(logprobs[index]))\
                .float().cuda()
            sampled_returns = Variable(torch.from_numpy(returns[index]))\
                .float().cuda()
            sampled_advs = Variable(torch.from_numpy(advantages[index]))\
                .float().cuda()

            new_value, new_logprob, dist_entropy =\
                policy.evaluate_actions(sampled_obs, sampled_actions)
            ...
            surrogate2 = torch.clamp(ratio, 1 - clip_value, 1 + clip_value) \
                * sampled_advs
            ...
            return policy_loss.data[0], value_loss.data[0], dist_entropy.data[0]


def generate_trajectory(env, policy, max_step, is_render=False, obs_fn=None,
                        progress=False):
    ...
    while not (nstep == max_step):
        ...
        value, action, logprob = value.data.cpu().numpy()[0], \
            action.data.cpu().numpy()[0], logprob.data.cpu().numpy()[0]
        ...
        nstep += 1
        if progress:
            print('\rStep: {}/{}'.format(nstep, max_step), flush=True, end='')
    if done:
        last_value = 0.0
    else:
        if obs_fn is not None:
